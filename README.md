...
> 后端 **947 条 pytest**、前端 **314 条 vitest + 1 条 Playwright**，全绿且**默认零 API 花费**。
...
| | 物理行 | 测试 |
|---|---|---|
| 后端（`app/` + `tests/`） | 约 **33,400** | pytest **947** 条（另有 6 条 `live` 默认不跑） |
...
| [问题记录.md](./问题记录.md) | 56 条按「现象→定位→根因→修复→验证」记录的工程问题 |
